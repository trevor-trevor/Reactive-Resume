...
import { templatesList, templatesMeta } from "@reactive-resume/utils";
        ...
        <div className="w-full overflow-hidden lg:absolute lg:right-0 lg:max-w-[55%]">
          ...
          >
            {templatesList.map((template) => {
              const displayName = templatesMeta[template]?.displayName ?? template;
              return (
                <motion.a
                  key={template}
                  target="_blank"
          ...
          {/* Fade-out overlay */}
          <div
            className="pointer-events-none absolute inset-y-0 left-0 hidden w-1/2
                       bg-gradient-to-r from-background to-transparent lg:block"
          />
        </div>
      </motion.div>
    </div>
